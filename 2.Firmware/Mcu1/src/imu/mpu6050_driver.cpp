...
void ARDUINO_ISR_ATTR InterruptFuntion()
{
  interrupt_flag = true;
}
...
void Mpu6050Driver::ResetMPU6050InterruptFlag()
{
    interrupt_flag = interrupt_flag_ = false;
}
...
bool Mpu6050Driver::MPU6050Init()
{
  ...
  if (I2cWrite(MPU6050_I2CADDR_DEFAULT, MPU6050_INT_PIN_CONFIG, 0x02) != ESP_OK) // logic level for the INT pin is active high
  {
    printf("IMU: 0x05 \r\n");
    return false;
  }

  if (I2cWrite(MPU6050_I2CADDR_DEFAULT, MPU6050_INT_ENABLE, 0x01) != ESP_OK) // able data ready interrupt
  {
    printf("IMU: 0x06 \r\n");
    return false;
  }

  if (I2cWrite(MPU6050_I2CADDR_DEFAULT, MPU6050_SMPLRT_DIV, 0x00) != ESP_OK) // Sample Rate: Gyro output rate / (1 + 0) = 1000Hz
  {
    printf("IMU: 0x07 \r\n");
    return false;
  }
        ...
        while(cal_gyro_offset_seccess == false )
        {
              ...
              fabs(offset_data.gyro.gz) < IMU_INIT_GYRO_THRESHOLD)
            {
                ...
                if(count > 1500)
                {
                    ...
                    printf("IMU: calibrate offset seccess, gx: %f gy: %f gz: %f\r\n",
                                                   gyro_offset_.gx,
                                                   gyro_offset_.gy,
                                                   gyro_offset_.gz);
                }

...
void CubliMiniControl::LinkSensor(AHRS *_ahrs) { ahrs_ = _ahrs; }
...
void CubliMiniControl::Init()
{
    ...
    printf(
        "WifiParam: use_wifi: %d ssid: %s password: %s server_port: %d ssid_len: %d password_len: "
        "%d\r\n",
...
float CubliMiniControl::LqrCalAxis(
    const AxisSensor_t &_sensor, const AxisParam_t &_param, float _axis_offset)
{
    return (_sensor.angle * cos(_axis_offset / 57.3f) - _param.angle_offset) * _param.kp +
           _sensor.gyro * cos(_axis_offset / 57.3f) * _param.kv + _sensor.speed * _param.ks;
}
...
void CubliMiniControl::Ch2AxisUBalance()
{
    ...
    if (fabs(u_ch2_balance_.sensor.angle - u_ch2_balance_.param.angle_offset) <
            protection_threshold_ &&
        cube_is_upside_dowm_ == false)
    {
...
void CubliMiniControl::PBalance()
{
        ...
        cube_is_upside_dowm_ == false)
    {
        ...
        GetMotorMoveSpeed(x_axis_speed, y_axis_speed, z_axis_speed, motor_set_speed_);
    }
    else
    {
        cube_is_upside_dowm_ = true;
...
void CubliMiniControl::PBalanceCalibrationAngle_Y(CalibrationMode_e &mode)
{
        ...
        fabs(motor_get_speed_.ch3 >= AUTO_CALIBRATION_SPEED_THRESHOLD))
    {
            ...
            motor_get_speed_.ch3 <= -AUTO_CALIBRATION_SPEED_THRESHOLD)
        {
            ...
            motor_get_speed_.ch1 <= -AUTO_CALIBRATION_SPEED_THRESHOLD &&
            motor_get_speed_.ch3 >= AUTO_CALIBRATION_SPEED_THRESHOLD)
        {
            p_balance_.param.y.angle_offset -= angle_scale;
        }
        ...
        fabs(get_speed) <= AUTO_CALIBRATION_SPEED_THRESHOLD &&
        fabs(motor_get_speed_.ch1 <= AUTO_CALIBRATION_SPEED_THRESHOLD) &&
        fabs(motor_get_speed_.ch3 <= AUTO_CALIBRATION_SPEED_THRESHOLD))
    {
        mode = eEND;
...
void CubliMiniControl::Control()
{
    ...
    {
        MotorStop();
        if (low_power_reminder_time_.WaitMs(1000) && systeam_low_power_ == true)
        {
            printf("low power!!\r\n");
        }
    }
...
void CubliMiniControl::ModeControl()
{
    ...
    break;
    case ePOINT_CALIBRATION:
        if (fabs(p_balance_.sensor.x.angle - p_balance_.param.x.angle_offset) <
                AUTO_CALIBRATION_STATIC_THRESHOLD &&
            fabs(p_balance_.sensor.y.angle - p_balance_.param.y.angle_offset) <
                AUTO_CALIBRATION_STATIC_THRESHOLD)
        {
            // 3s
...
void CubliMiniControl::ResetCanQueue() { bsp_driver_.can_.ResetQueue(); }

void CubliMiniControl::SetCh1MotorInitResult(uint8_t _result) { motor_init_status_.ch1 = _result; }
...
void CubliMiniControl::WifiWriteDefaultParam()
{
    ...
    wifi_param_.wifi_password_len =
        strlen(passwork) < WIFI_CHAR_LEN ? strlen(passwork) : WIFI_CHAR_LEN;
